...
This project includes JUnit assertions that compare object references in Java:

✅ assertThat(actual).isEqualTo(expected) — checks that two variables refer to the same object instance.

❌ assertThat.(actual)isNotEqualTo(unexpected) — checks that two variables refer to different object instances.

These assertions are useful when it's important to ensure identity (not just equality) between objects in memory during testing.
