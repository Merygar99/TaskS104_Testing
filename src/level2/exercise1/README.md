...
This project includes examples of JUnit assertions that test the value comparison of two Integer objects in Java:

✅ assertThat(actual).isEqualTo(expected) — verifies that two integer values are equal.

❌ assertThat(actual).isNotEqualTo(unexpected) — verifies that two integer values are not equal.

These assertions are essential in unit testing to validate logic, especially when comparing primitive wrapper objects like Integer.
